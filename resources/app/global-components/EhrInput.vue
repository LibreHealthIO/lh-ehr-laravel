<template>
    <div :class="error ? 'has-error' : ''" class="flex flex-col capitalize">
        <ehr-label v-if="label" :id="id" :required="required" :label="label" />
        <div v-if="type === 'text-area'" class="relative">
            <textarea
                ...
                :class="customClass" :disabled="disabled" :readonly="read_only" @input="$emit('input', $event.target.value)"
                :id="id" :cols="cols" :rows="rows" v-bind="$attrs" :minlength="minLength" :maxlength="maxLength"
                :placeholder="placeholder" :required="required">{{ value }}</textarea>
        </div>
        <div v-else-if="type === 'select'" class="relative">
            <select :disabled="disabled" :required="required" @input="$emit('input', $event.target.value)"
                ...
                <option select class="text-gray-100">{{ placeholder ? placeholder : 'Choose a value' }}</option>
        ...
        <div v-else class="relative">
            <input :id="id" ref="input" :disabled="disabled" :readonly="read_only" v-bind="$attrs" :minlength="minLength"
                ...
                :type="this.isPassword ? this.togglePassword ? 'password' : 'text' : this.type" :value="value"
                ...
                :class="customClass" @input="$emit('input', $event.target.value)" />

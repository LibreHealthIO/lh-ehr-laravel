...
<!-- class="dropdown-menu absolute border-t-2 border-theme-1 py-1 w-48 bg-white
shadow-lg z-50" -->
<script>
export default {
    name: "EhrHeaderMenu",
    ...
    methods: {
        ...
        isActiveClass: function (url) {
            return this.isUrl(url)
                ...
                : this.subMenu
                ? "mr-1 ml-1 px-3 py-2 rounded-md text-sm font-small text-black hover:text-black-800 " +
                  "inline-flex items-center bg-white-800 hover:bg-white-700 focus:outline-none focus:text-black focus:bg-white-700 " +
                  "transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-100 cursor-pointer inset-x-full"
                : "mr-1 ml-1 px-2 py-2 rounded-md text-xs font-small text-gray-300 hover:text-white " +
        ...
        isSubMenuActive: function (url) {
            return this.isUrl(url) && this.subMenu
                ? "block px-4 py-2 text-sm text-gray-700 bg-white hover:bg-white"

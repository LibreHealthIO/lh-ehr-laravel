    ...
    public function up()
    {
        Schema::create('background_services', function (Blueprint $table) {
	        ...
	        $table->boolean('active')->default(0)
                ->comment('Is service active or deleted.  0 -> False | 1 -> True');
	        $table->boolean('running')->default(0)
                ->comment('Is service running or stopped. 0 -> Stopped(False) | 1 -> Running(True)');
	        $table->timestamp('next_run')->useCurrent()->comment('When next run is scheduled?');
	        $table->integer('execute_interval', 0)->unsigned()->default(0)
	        ...
	        $table->integer('sort_order')->unsigned()
                ->comment('If there are multiple services, then lower number will run first.');
            $table->timestamps();
        });
    }

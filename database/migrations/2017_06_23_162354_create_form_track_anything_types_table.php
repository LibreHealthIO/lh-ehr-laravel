...
class CreateFormTrackAnythingTypesTable extends Migration
{
     ...
     * From UI, Select Patient -> Encounter -> Miscellaneous -> Track Anything ->
     * Configure Tracks -> Create New Track. | Select Patient -> Encounter ->
     * Patient/Client -> Visit Forms -> Track Anything -> Configure Tracks -> Create New Track.
    ...
    public function up()
    {
        Schema::create('form_track_anything_types', function (Blueprint $table) {
	        ...
	        $table->unsignedInteger('parent')->default(0)
                ->comment('Parent of any type. This happens in the case when you edit the type and enters
                a position of another type. Initially it will contain 0 as default value, indicating no parent.');
	        $table->unsignedInteger('position')->default(0)
                ->comment('At which position it should appear in the list.');

	        $table->boolean('active')->default(0)
                ->comment('Whether this type is enabled or not. Disable from Configure Track.
                 0 -> False | 1 -> True.');
            $table->timestamps();
        });
    }

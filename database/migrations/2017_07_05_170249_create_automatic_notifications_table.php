...
class CreateAutomaticNotificationsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('automatic_notifications', function (Blueprint $table) {
	        ...
	        $table->string('sms_gateway_type') // TODO implement sms notifications
                ->comment('Type of sms gateway.');
	        $table->dateTime('next_app_schedule')
                ->comment('When next notification is scheduled?');
	        $table->string('provider_name');
	        $table->text('message')->nullable()
                ->comment('Message to be sent.');
	        $table->string('email_sender')
                ->comment('Who sent email? It can be any user or any clinic.');
	        $table->string('email_subject')
                ->comment('Subject of notification');
	        $table->enum('type', ['sms', 'email'])->default('SMS')
                ->comment('Type of notification.');
	        $table->dateTime('notification_sent_date')->useCurrent()

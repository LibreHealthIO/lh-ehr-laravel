...
class CreateX12PartnersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('x12_partners', function (Blueprint $table) {
	        ...
	        $table->enum('processing_format', ['standard', 'medi-cal', 'cms', 'proxymed'])
                ->comment('Processing Format');
	        $table->string('x12_isa_01', 2)->default('00')
                ->comment('User Logon Required Indicator');
	        $table->string('x12_isa_02')->nullable()->comment('User Logon. If 03 in x12_isa_01');
	        $table->string('x12_isa_03', 2)->default('00')
                ->comment('User Password required indicator.');
	        $table->string('x12_isa_04')->nullable()->comment('User Password. If 01 in x12_isa_03.');
	        $table->string('x12_isa_05', 2)->default('ZZ')
                ->comment('Sender Id Qualifier.');
	        $table->string('x12_isa_07', 2)->default('ZZ')
                ->comment('Receiver Id Qualifier');
	        $table->boolean('x12_isa_14')->default(0)
                ->comment('Acknowledgement Requested. 0 -> No | 1 -> Yes');
	        $table->string('x12_isa_15', 1)->default('P')
                ->comment('Usage Indicator. viz Production(P) and Testing(T)');
